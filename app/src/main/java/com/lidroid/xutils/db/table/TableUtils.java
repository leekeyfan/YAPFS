...
public class TableUtils {
     ...
     * key: entityType.name
     */
    private static ConcurrentHashMap<String, HashMap<String, com.lidroid.xutils.db.table.Column>> entityColumnsMap = new ConcurrentHashMap<String, HashMap<String, com.lidroid.xutils.db.table.Column>>();

    /* package */
    static synchronized HashMap<String, com.lidroid.xutils.db.table.Column> getColumnMap(Class<?> entityType) {

        if (entityColumnsMap.containsKey(entityType.getName())) {
            return entityColumnsMap.get(entityType.getName());
        }

        HashMap<String, com.lidroid.xutils.db.table.Column> columnMap = new HashMap<String, com.lidroid.xutils.db.table.Column>();
    ...
    private static void addColumns2Map(Class<?> entityType, String primaryKeyFieldName, HashMap<String, com.lidroid.xutils.db.table.Column> columnMap) {
            ...
            for (Field field : fields) {
                ...
                if (ColumnConverterFactory.isSupportColumnConverter(field.getType())) {
                    if (!field.getName().equals(primaryKeyFieldName)) {
                        com.lidroid.xutils.db.table.Column column = new com.lidroid.xutils.db.table.Column(entityType, field);
                        if (!columnMap.containsKey(column.getColumnName())) {
                            columnMap.put(column.getColumnName(), column);
                        }
                    }
                } else if (ColumnUtils.isForeign(field)) {
                    com.lidroid.xutils.db.table.Foreign column = new com.lidroid.xutils.db.table.Foreign(entityType, field);
                    if (!columnMap.containsKey(column.getColumnName())) {
                        columnMap.put(column.getColumnName(), column);
                    }
                } else if (ColumnUtils.isFinder(field)) {
                    com.lidroid.xutils.db.table.Finder column = new Finder(entityType, field);
                    if (!columnMap.containsKey(column.getColumnName())) {
                        columnMap.put(column.getColumnName(), column);
                    }
                }

...
class HttpMultipart {
    ...
    private static void writeBytes(
        ...
        ByteArrayBuffer b = encode(com.lidroid.xutils.http.client.multipart.MIME.DEFAULT_CHARSET, s);
        writeBytes(b, out);
    }

    private static void writeField(
            final com.lidroid.xutils.http.client.multipart.MinimalField field, final OutputStream out) throws IOException {
    ...
    private static void writeField(
            final com.lidroid.xutils.http.client.multipart.MinimalField field, final Charset charset, final OutputStream out) throws IOException {
    ...
    private static final ByteArrayBuffer FIELD_SEP = encode(com.lidroid.xutils.http.client.multipart.MIME.DEFAULT_CHARSET, ": ");
    private static final ByteArrayBuffer CR_LF = encode(com.lidroid.xutils.http.client.multipart.MIME.DEFAULT_CHARSET, "\r\n");
    private static final ByteArrayBuffer TWO_DASHES = encode(com.lidroid.xutils.http.client.multipart.MIME.DEFAULT_CHARSET, "--");


    private String subType;
    private final Charset charset;
    private final String boundary;
    private final List<com.lidroid.xutils.http.client.multipart.FormBodyPart> parts;
    ...
    public HttpMultipart(final String subType, final Charset charset, final String boundary, HttpMultipartMode mode) {
        ...
        this.charset = charset != null ? charset : com.lidroid.xutils.http.client.multipart.MIME.DEFAULT_CHARSET;
        this.boundary = boundary;
        this.parts = new ArrayList<com.lidroid.xutils.http.client.multipart.FormBodyPart>();
        this.mode = mode;
    }
    ...
    public List<com.lidroid.xutils.http.client.multipart.FormBodyPart> getBodyParts() {
        return this.parts;
    }

    public void addBodyPart(final com.lidroid.xutils.http.client.multipart.FormBodyPart part) {
        if (part == null) {
            return;
        }
        this.parts.add(part);
    ...
    private void doWriteTo(
        ...
        for (com.lidroid.xutils.http.client.multipart.FormBodyPart part : this.parts) {
            ...
            switch (mode) {
                case STRICT:
                    for (com.lidroid.xutils.http.client.multipart.MinimalField field : header) {
                        writeField(field, out);
                        callBackInfo.pos += encode(com.lidroid.xutils.http.client.multipart.MIME.DEFAULT_CHARSET,
                ...
                case BROWSER_COMPATIBLE:
                    ...
                    // Use content charset
                    com.lidroid.xutils.http.client.multipart.MinimalField cd = header.getField(com.lidroid.xutils.http.client.multipart.MIME.CONTENT_DISPOSITION);
                    writeField(cd, this.charset, out);
                    callBackInfo.pos += encode(this.charset,
                            cd.getName() + cd.getBody()).length() + FIELD_SEP.length() + CR_LF.length();
                    String filename = part.getBody().getFilename();
                    if (filename != null) {
                        com.lidroid.xutils.http.client.multipart.MinimalField ct = header.getField(MIME.CONTENT_TYPE);
